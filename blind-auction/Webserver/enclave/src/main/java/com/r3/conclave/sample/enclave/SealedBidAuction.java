...
public class SealedBidAuction extends Enclave {
     ...
     *   Step 1: Create a variable called "allBids" of type ArrayList which we will use to store the various MAILs.
     ...
     *   Step 2: Next let's define a method that can receive byte[]'s as input and return byte[]'s as output. Let's give
     *   a public access modifier and name it "invoke".
     *
     *   Step 3: Upon receiving a new bid, we should first add it to our existing LIST "allBids". We can later use the
     ...
     *
     *   Step 4: Now that we have a LIST that is growing in size each time a bid is sent from a client, let's add an
     *   IF statement which will check to see if the current allBids.size() is 5 yet.
     *
     *   Step 5: Assuming the IF statement we wrote in the previous step, we have to implement logical block that should
     ...
     *
     *   Step 6: Finally, we have to return the highest bid value that we found as an array of bytes.
     *
    ...
    protected void receiveMail(long id, String routingHint, EnclaveMail mail) {
         ...
         *
         *  HINT: Examine the [createMail] function to see how you can leverage the parameters available to this method to create the
         *  MutableMail object.
         *
